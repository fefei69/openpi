#!/usr/bin/env bash
# Start the pi0.5 dense contract-five checkpoint for dense_client.py; arguments go to serve_dense.py.
set -euo pipefail
...
exec "$hanoi_python" -m examples.hanoi.deployment.serve_dense "$@"
