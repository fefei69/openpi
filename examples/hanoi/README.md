...
This integration uses a Trossen WXAI arm, one external ROS RGB camera, and the recorded noisy 30 Hz Cartesian
references. The full design and queue policy are in [the training plan](../../docs/hanoi_training_plan.md).
Live driver integration and hardware trials are deferred. `execution.py` creates command descriptions and moves no robot.
...
- The persistent manager is active in the existing CPU allocation. Its host PID and allocation expiration are recorded
  ...
  [dated operational progress](../../data/hanoi/PROGRESS.md) for later observations.
...
CPU logger `examples.hanoi.wandb_logger`. It publishes loss, gradient/parameter norms, allocated-GPU telemetry and
...
JAX_PLATFORMS=cpu .venv/bin/python -m examples.hanoi.wandb_logger \
...
.venv/bin/python -m examples.hanoi.convert_hanoi_data_to_lerobot --resume
.venv/bin/python -m examples.hanoi.compute_norm_stats
.venv/bin/python -m examples.hanoi.verify_execution
.venv/bin/python -m examples.hanoi.verify_data
```
...
`prepare.sbatch`. Its final gate checks every converted state/action row, sampled exact RGB/chunk values, episode
...
`qualify.sbatch` runs 100 measured full training steps after warmup, saves/restores optimizer state, checks another
optimizer step, restored EMA evaluation and inference, and the native serving factory. It records throughput, GPU
memory, and actual checkpoint sizes. `train.sbatch` checks preparation, normalization, selection, environment, and
...
sbatch --account=torch_pr_595_tandon_advanced --nodes=1 --ntasks=1 \
  ...
  examples/hanoi/qualify.sbatch --profile h100_1 --exp-name qualification_h100_1 \
...
JAX_PLATFORMS=cpu .venv/bin/python -m examples.hanoi.manage --exp-name hanoi_20260914
# Run an owned controller in an appropriate CPU session when no controller is active.
JAX_PLATFORMS=cpu .venv/bin/python -m examples.hanoi.manage --exp-name hanoi_20260914 --submit --watch
```
...
`evaluate.sbatch` evaluates all eligible validation anchors, including the last partial batch, and samples up to 64
...
Before completion, `verify_serving.py` reopens the selected checkpoint through `create_trained_policy` and compares
...
```bash
.venv/bin/python -m examples.hanoi.evaluate --config-name pi05_hanoi_multitask --exp-name hanoi_20260914
...
```bash
source examples/hanoi/env.sh
JAX_PLATFORMS=cpu .venv/bin/python -m examples.hanoi.finalize --exp-name hanoi_20260914 --watch
```
...
```bash
source examples/hanoi/env.sh
JAX_PLATFORMS=cpu .venv/bin/python -m examples.hanoi.deliver --exp-name hanoi_20260914
```
...
```bash
source examples/hanoi/env.sh
JAX_PLATFORMS=cpu .venv/bin/python -m pytest src/openpi/policies/hanoi_policy_test.py \
  src/openpi/training/data_loader_test.py src/openpi/training/checkpoints_test.py \
  examples/hanoi -k 'not with_real_dataset' -q
```
