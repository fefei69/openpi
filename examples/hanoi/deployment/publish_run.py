...
* for Cosmos runs, ``dreams.mp4``, ``dream_strips/``, ``dreams_contact.png``, ``dream_report.json``
  (regenerated from the saved inputs with ``examples/hanoi/dream_dense_run.py`` in the Cosmos checkout)
...
LEVELS = np.array([57.1, 67.8, 77.5, 87.7])


def peg(y_m: float) -> str:
    return "A" if y_m < -0.02 else ("B" if y_m < 0.05 else "C")


def reconstruct_moves(events: list) -> dict:
    """Ring moves implied by the gripper events, tracked from the standard start A = [4, 3, 2, 1]."""
    t0 = events[0]["monotonic_s"]
    stacks = {"A": [4, 3, 2, 1], "B": [], "C": []}
    held = None
    moves, grasps = [], []
    for e in events:
        if e["event"] != "command" or e["kind"] != "gripper":
            continue
        x = np.asarray(e["target_xyz_m"]) * 1000
        p = peg(x[1] / 1000)
        t = round(e["monotonic_s"] - t0, 1)
        if not e["jaw_open"]:
            ring = stacks[p][-1] if stacks[p] else None
            level = float(LEVELS[np.abs(LEVELS - x[2]).argmin()])
            grasps.append({"t_s": t, "peg": p, "ring": ring, "z_mm": round(float(x[2]), 1), "level_error_mm": round(float(x[2] - level), 1)})
            held = (ring, p)
            if ring:
                stacks[p].pop()
        else:
            ring, src = held if held else (None, None)
            legal = ring is not None and (not stacks[p] or stacks[p][-1] > ring)
            if ring is not None:
                stacks[p].append(ring)
            moves.append({"t_s": t, "ring": ring, "from": src, "to": p, "legal": legal, "board": {k: list(v) for k, v in stacks.items()}})
            held = None
    return {"moves": moves, "grasps": grasps, "final_board": stacks, "held_at_end": held,
            "solved": stacks == {"A": [], "B": [], "C": [4, 3, 2, 1]}, "all_legal": all(m["legal"] for m in moves)}
...
def main():
    ...
    moves = reconstruct_moves(events)
    ...
    if bag.is_dir() and any(bag.glob("*.mcap")) and (summary.get("camera_bag") or {}).get("outcome"):
        subprocess.run([sys.executable, "-m", "examples.hanoi.deployment.bag_to_video", str(run), "--output", str(folder / "camera.mp4")],
