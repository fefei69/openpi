#!/usr/bin/env bash
# Run the Cosmos waypoint policy client: initialize the robot, then commit one
# predicted destination at a time for 60 seconds by default. The server is
# cosmos_policy.experiments.robot.hanoi.serve_waypoint in the Cosmos checkout.
set -euo pipefail
...
exec "$hanoi_python" -m examples.hanoi.deployment.cosmos_client "${hanoi_args[@]}"
