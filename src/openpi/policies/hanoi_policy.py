"""Shared input and output contract for the single-arm Cartesian Hanoi policy."""
...
class HanoiInputs(transforms.DataTransformFn):
    def __call__(self, data: dict) -> dict:
        image = parse_image(data["observation/image"])
        state = np.asarray(data["observation/state"], dtype=np.float32)
        if state.shape != (7,) or not np.isfinite(state).all():
            raise ValueError("Hanoi state must contain exactly seven finite measured values")
        inputs = {
            "state": state.copy(),
            "image": {
                "base_0_rgb": image,
        ...
        if "actions" in data:
            ...
            if actions.ndim != 2 or actions.shape[-1] != 4 or not np.isfinite(actions).all():
                raise ValueError("Hanoi targets must be a finite (horizon, 4) array")
...
@dataclasses.dataclass(frozen=True)
class HanoiOutputs(transforms.DataTransformFn):
    def __call__(self, data: dict) -> dict:
